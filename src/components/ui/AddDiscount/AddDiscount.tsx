...
const validationSchema = Yup.object({
  ...
  fechaLimite: Yup.string()
    ...
    .matches(/^\d{4}-\d{2}-\d{2}$/, "Formato inválido")
    .test("is-after", "La fecha límite debe ser posterior a la de inicio", function (value) {
      const { fechaInicio } = this.parent;
      return new Date(value) > new Date(fechaInicio);
    }),
});

const AddDiscount = ({ onClose, initialValues = defaultValues }: AddDiscountProps) => {
  return (
    <Formik<IDescuento>
        ...
        <Form>
          <div className="w-full max-w-md border border-black p-6 bg-white">
            <div className="text-center text-2xl font-bold mb-6">
              <h1>{initialValues.id ? "Editar descuento" : "Agregar descuento"}</h1>
            </div>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-col items-center">
                ...
                {touched.descuento && errors.descuento && (
                  <p className="text-red-500 text-sm mt-1">{errors.descuento}</p>
                )}
              </div>
