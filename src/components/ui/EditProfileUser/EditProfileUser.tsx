...
const EditProfileUser = ({ initialData }: EditProfileUserProps) => {
  ...
  const validationSchema = Yup.object({
    ...
    password: Yup.object({
      ...
      nueva: Yup.string().min(6, "Mínimo 6 caracteres").required("Ingrese una nueva contraseña"),
      repetir: Yup.string()
      ...
      {({ values, errors, touched, handleChange, handleSubmit, handleBlur, setFieldValue }) => (
        <Form onSubmit={handleSubmit}>
            <div>
                        ...
                        <div className="absolute top-full left-0 bg-white p-4 border border-gray-300 rounded-md shadow-md w-full z-10">
                          ...
                          {touched.direcciones?.[0]?.localidad && 
                            ...
                            errors.direcciones?.[0]?.localidad && (
                              <div className="text-red-500 text-sm">
                                {(errors.direcciones[0] as FormikErrors<IDireccion>).localidad}
                              </div>
                          )}
                          <input
                          ...
                          {touched.direcciones?.[0]?.calle && 
                            ...
                            errors.direcciones?.[0]?.calle && (
                              <div className="text-red-500 text-sm">
                                {(errors.direcciones[0] as FormikErrors<IDireccion>).calle}
                              </div>
                          )}
                          <input
                          ...
                          {touched.direcciones?.[0]?.cp && 
                            ...
                            errors.direcciones?.[0]?.cp && (
                              <div className="text-red-500 text-sm">
                                {(errors.direcciones[0] as FormikErrors<IDireccion>).cp}
                              </div>
                          )}
                        </div>
                      )}
              ...
              {pwdOpen && (
                <ChangePasswordModal onClose={() => setPwdOpen(false)} onSubmit={({ contraseñaActual, nuevaContraseña }) => {
